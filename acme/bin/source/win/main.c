...
mainctl(void *v)
{
	...
	for(;;){
		...
		case 'S':	/* output to stdout */
			snprint(tmp, sizeof tmp, "#%lud", hostpt);
			if(winsetaddr(w, tmp, 1) == 0){
				h0 = hostpt;
				wingetaddr(w, &hostpt, &hostpt);
				fprint(2, "hostpt reset %lud → %lud\n", h0, hostpt);
			}
