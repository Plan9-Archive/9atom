...
winsetaddr(Window *w, char *addr, int errok)
{
	...
	if(write(w->addr, addr, strlen(addr)) < 0){
		if(!errok)
			error("error writing addr(%s): %r from %#p", addr, getcallerpc(&w));
		return 0;
	}
	return 1;
}
...
int
wingetaddr(Window *w, ulong *p0, ulong *p1)
{
	char buf[128], *r;
	int n;
	...
	if(w->addr < 0)
		w->addr = winopenfile(w, "addr");
	n = read(w->addr, buf, sizeof buf - 1);
	if(n < 0)
		error("error reading addr: %r from %#p", getcallerpc(&w));
	buf[n] = 0;
	*p0 = strtoul(buf, &r, 0);
	*p1 = strtoul(buf, &r, 0);
	return 1;
