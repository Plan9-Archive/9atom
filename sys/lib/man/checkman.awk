...
END {
	...
	for (i in Refs) {
		...
		if (i !~ /^[1-9][a-z]*\/intro$/) {
			split(tolower(i), a, "/")
			print "grep -n " a[2] ".*" a[1] " [0-9]*/* # Need " tolower(i)
		}
	}
	print ""
...
#	getnmlist("/386/lib/libsunrpc.a")
	getnmlist("/386/lib/libthread.a")
	getnmlist("/386/lib/libventi.a")
	getnmlist("/386/lib/libxml.a")
	for (i in List) {
