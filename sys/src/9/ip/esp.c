...
struct Window
{
	uchar	w0set;		/* unknown how this is properly set */
	ulong	w0;
	uvlong	w;
};
...
void
espstat(Proto *esp, int stat, uint n)
{
	Esppriv *p;
...
static int
ckwindow(Proto *esp, Window *w, ulong seq)
{
	long δ;
	...
	if(δ < 0 || w->w & 1ull << 63 - δ)
		return -1;
	if(δ > 63){
		netlog(esp->f, Logesp, "long drop seq %lud %lud\n", seq, w->w0);
...
print("esp: cipher failed %I -> %I!%lud: %s\n", vers.raddr, vers.laddr, vers.spi, up->errstr);
	...
	if(ckwindow(esp, ecb, vers.seq) == -1){
		...
		qunlock(c);
print("esp: window fail %I -> %I!%lud: %lux %lux %.64llub\n", vers.raddr, vers.laddr, vers.spi, vers.seq, ecb->w0, ecb->w);
		netlog(f, Logesp, "esp: window fail %I -> %I!%lud: %lux %lux  %.64llub\n",
			vers.raddr, vers.laddr, vers.spi, vers.seq, ecb->w0, ecb->w);
		freeb(bp);
		return;
	}
