...
struct Rudpstats
{
	uvlong	rudpInDatagrams;
	uvlong	rudpNoPorts;
	uvlong	rudpInErrors;
	uvlong	rudpOutDatagrams;
};
...
struct Rudppriv
{
	...
	uvlong	rxmits;			/* # of retransmissions */
	uvlong	orders;			/* # of out of order pkts */
...
rudpstats(Proto *rudp, char *buf, int len)
{
	...
	upriv = rudp->priv;
	return snprint(buf, len, "%llud %llud %llud %llud %llud %llud\n",
		upriv->ustats.rudpInDatagrams,
		upriv->ustats.rudpNoPorts,
		upriv->ustats.rudpInErrors,
		upriv->ustats.rudpOutDatagrams,
