...
_rtctime(void)
{
	...
	if(t & Rtc12){
		rtc.hour = bcd2dec((t & Rtchr12) >> Rtchrs) - 1;	/* 1 to 12 */
		if(t & Rtcpm)
			rtc.hour += 12;
	}else
		rtc.hour = bcd2dec((t & Rtchr24) >> Rtchrs);	/* 0 to 23 */

	rtc.mday = bcd2dec(d & Rdmday);				/* 1 to 31 */
	rtc.mon = bcd2dec((d & Rdmon) >> Rdms);		/* 1 to 12 */
