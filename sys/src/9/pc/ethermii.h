...
int	mii(Mii*, int);		/* depricated */
Mii*	miiattach(void*, int, int (*)(Mii*, int, int), int (*)(Mii*, int, int, int));
void	miidetach(Mii* mii);
int	miiane(Mii*, int, int, int);
char*	miidumpphy(Mii*, char*, char*);
int	miimir(Mii*, int);
int	miimiw(Mii*, int, int);
int	miireset(Mii*);
int	miistatus(Mii*);
