...
//	{ "GA620", ga620pnp, 0, },
	...
	{ "yuk", yukpnp, 0, },
//	{ "bcm57xx", bcmpnp, 0, },		/* nukes file server */
...
toringbuf(Ether *e, void *u, int len)
{
	...
	if (rb->owner == Interface){
		...
		h = 0;
		for(i = 0; i < e->nrb; i++)
			if(e->rb[i].owner == Host)
				h++;
		if((freq++ & 0xfff) == 0)
			print("%d: toringbuf: drop @ %d [%d %d]\n", e->ctlrno, e->ri, h, e->nrb);
	}
}
