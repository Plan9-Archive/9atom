...
int
tsdbio(SDunit *unit, SDpart *part, void *a, uvlong off, int mbr)
{
	uchar *b;

	if(sdbio(unit, part, a, unit->secsize, off) != unit->secsize){
		if(trace)
			print("%s: read %lud at %llud failed\n", unit->name,
				unit->secsize, part->start*unit->secsize+off);
		return -1;
	}
	b = a;
	if(mbr && (b[0x1FE] != 0x55 || b[0x1FF] != 0xAA)){
		if(trace)
			print("%s: bad magic %.2ux %.2ux at %llud\n",
				unit->name, b[0x1FE], b[0x1FF],
				part->start*unit->secsize+off);
		return -1;
	}
	return 0;
}
 ...
 */
static char omagic[] = "plan9 partitions";
...
oldp9part(SDunit *unit)
{
	...
	if(strncmp((char*)partbuf, omagic, sizeof(omagic)-1) != 0) {
	...
	if(n && strncmp(line[0], omagic, sizeof(omagic)-1) == 0){
...
isdos(int t)
{
	return t==FAT12 || t==FAT16 || t==FAT16EXT || t==FATHUGE ||
		t==FAT32 || t==FAT32X;
}
...
partition(SDunit *unit)
{
	...
	else if((type & NEW) && p9part(unit, "data")  >= 0){
		/* nothing to do */
	}
	else if(type & OLD)
		oldp9part(unit);
