...
static SDev*
ataprobe(int cmdport, int ctlport, int irq, int)
{
	Ctlr* ctlr;
	SDev *sdev;
	Drive *drive;
...
extern	int	fmtstrcpy(Fmt*, char*);
	...
	tbdf = va_arg(f->args, uint);
	if(tbdf == BUSUNKNOWN)
		return fmtstrcpy(f, "unknown");
	type = BUSTYPE(tbdf);
	p = buf;
	e = p + sizeof buf;
//	if(type < nelem(bustypes))
//		p = seprint(p, e, "%s", bustypes[type]);
//	else
...
atapnp(void)
{
	Ctlr *ctlr;
	Pcidev *p;
	int channel, map, ispc87415, pi, r, tbdf;
	SDev *sdev, *head, *tail;
	void (*irqack)(Ctlr*);
	...
	for(p = nil; p = pcimatch(p, 0, 0); ){
		...
		case (0x2368<<16)|0x197b:	/* jmicron jmb368 */
			print("sdata: warning: untested jmb %.3ux\n", p->did & 0xfff);
			break;
		...
		case (0x2850<<16)|0x8086:	/* 82801HBM/HEM (ICH8M/ICH8M-E) IDE */
	...
	if(lchan[0].probed + lchan[1].probed == 0)
		for(channel = 0; channel < 2; channel++){
			...
			if(lchan[channel].probed == 0){
				print("sdata: blind probe %.3ux", lchan[channel].cmdport);
				sdev = ataprobe(lchan[channel].cmdport,
					lchan[channel].ctlport, lchan[channel].irq, 3);
				if(sdev != nil)
					print("..found");
...
{
	return ataprobe(port, port+0x204, irq, 3);
}
