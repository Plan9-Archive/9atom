...
out:
	...
	if(x){
		regalloc(&nod3, &regnode, Z);
		nod1.xoffset += SZ_LONG*w;
		nod2.xoffset += SZ_LONG*w;
		nod1.type->etype = types[TUSHORT]->etype;
		nod2.type->etype = types[TUSHORT]->etype;
		for(; x >= SZ_SHORT; x -= SZ_SHORT){
			gmove(&nod1, &nod3);
			gmove(&nod3, &nod2);
			nod1.xoffset += SZ_SHORT;
			nod2.xoffset += SZ_SHORT;
		}
		nod1.type->etype = types[TUCHAR]->etype;
		nod2.type->etype = types[TUCHAR]->etype;
		for(; x >= SZ_CHAR; x -= SZ_CHAR){
			gmove(&nod1, &nod3);
			gmove(&nod3, &nod2);
			nod1.xoffset += SZ_CHAR;
			nod2.xoffset += SZ_CHAR;
		}
		regfree(&nod3);
	}
