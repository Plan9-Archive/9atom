...
oclass(Adr *a)
{
	...
	if(a->type < D_CONST2 && (a->type >= D_INDIR || a->index != D_NONE)) {
...
asmand(Adr *a, int r)
{
	...
	if(t >= D_X0 && t <= D_X7 || t >= D_Y0 && t <= D_Y7) {
		if(v)
			goto bad;
		*andptr++ = (3 << 6) | (reg[t] << 0) | (r << 3);
		if(t >= D_Y0 && t <= D_Y7)
...
//note: 4th register encoding in immediate byte
	...
	if(np+1 < n) {
		...
		if(and[np] != 0x0F) {
			fprint(2, "internal: inconsistent vex state: %P\n", p);
			return;
		}
		np++;
		if(np < n) {
