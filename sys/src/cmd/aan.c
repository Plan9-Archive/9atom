...
/* record fmt escapes the box, so we must know its size */
typedef struct {
...
static int		writen(int, void *, int);
...
static void		marshal(Hdr*, Mhdr*);
static void		unmarshal(Hdr*, Mhdr*);
...
fromnet(void*)
{
	...
	while (!done) {
		...
		// Read the header.
		if ((len = readn(netfd, &mhdr, Hdrsz)) <= 0) {
			if (len < 0)
...
static int
writen(int fd, void *vbuf, int nb)
{
	uchar *buf;
	int len = nb;
...
static void
marshal(Hdr *h, Mhdr *m)
{
	putle(m->nb, h->nb, 4);
	putle(m->msg, h->msg, 4);
	putle(m->acked, h->acked, 4);
}

static void
unmarshal(Hdr *h, Mhdr *m)
{
	h->nb = getle(m->nb, 4);
	h->msg = getle(m->msg, 4);
	h->acked = getle(m->acked, 4);
}
