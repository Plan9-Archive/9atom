...
static char seq[NumFnt*2] = "trsrnrlrvrtisinilivitbsbnblbvbttstntltvt";
...
initfontpaths(void)
{
	...
	p = getenv("htmlfont");
	if(p == nil)
		sysfatal("abaco: $htmlfont unset");
	if((b = Bopen(p, OREAD)) == nil)
		sysfatal("abaco: no html fonts");
	free(p);
	for(; s = Brdstr(b, '\n', 1); free(s)){
		if(p = strchr(s, '#'))
			*p = 0;
		if(tokenize(s, f, nelem(f)) != 2 || strlen(f[0]) != 2)
			continue;
		f[0][0] = tolower(f[0][0]);
		f[0][1] = tolower(f[0][1]);
		p = strstr(seq, f[0]);
		...
		if(p == nil || i & 1)
			continue;
		free(fontpaths[i/2]);
		fontpaths[i/2] = strdup(f[1]);
	}
	Bterm(b);
	for(i = 0; i < NumFnt; i++)
		if(fontpaths[i] == nil || access(fontpaths[i], AREAD))
			sysfatal("abaco: bad font %#.2s %s\n", seq+i*2, fontpaths[i]);
//		else
//			fprint(2, "font%d: %#.2s %s\n", i, seq+i*2, fontpaths[i]);
}
...
ucvt(Rune* s)
{
	...
	for(i=0; i<n; i++){
		...
		if(inclass(c, L"-/$_@.!*'(),a-zA-Z0-9"))
			...
			l = snprint(buf, 16, "%C", c);
			for(k = 0; k < l; k++)
				j += snprint(u+j, 5, "%%%uhhx", buf[k]);
		}
	}
	u[j] = 0;
	return u;
...
latin1:
	...
	t = s;
	memset(&w, 0, sizeof w);
	w.sync = chancreate(sizeof(ulong), 0);
	if(w.sync == nil)
		error("can't create channel");
	w.fd = e.p[1];
	w.s = s;
	w.np = *np;
	proccreate(writeproc, &w, STACK);

	s = nil;
	while((n = read(e.q[0], buf, sizeof(buf))) > 0){
		s = erealloc(s, i+n+1);
		memmove(s+i, buf, n);
		i += n;
		s[i] = '\0';
	...
	n = recvul(w.sync);
	if(n != *np)
		fprint(2, "tcs: did not write %ld; wrote %lud\n", *np, n);
