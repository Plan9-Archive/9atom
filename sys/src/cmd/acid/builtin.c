...
extern Type* srch(Type *t, char *s);

void
acidoffsetof(Node *r, Node *args)
{
	Lsym *s;
	Node *av[Maxarg], res, res1;
	Type *t;

	na = 0;
	flatten(av, args);
	if(na != 2)
		error("offsetof(aggr, tag)");

	expr(av[0], &res);
	if(res.type == TSTRING){
		s = look(res.string->string);
		if(s == nil)
			error("unknown aggr %s", res.string->string);
		expr(av[1], &res1);
		if(res1.type != TSTRING)
			error("bad tag %s", res1.string->string);
		t = srch(s->lt, res1.string->string);
	}else{
		error("%s is not a complex type", res.sym->name);
		t = nil;		/* compiler */
	}
