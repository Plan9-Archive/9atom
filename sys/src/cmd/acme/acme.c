...
threadmain(int argc, char *argv[])
{
	...
	fontnames[0] = deffontname("vfont", fontnames[0]);
	fontnames[1] = deffontname("ffont", fontnames[1]);
	...
	case 'f':
		free(fontnames[0]);
		fontnames[0] = estrdup(EARGF(usage()));
		break;
	case 'F':
		free(fontnames[1]);
		fontnames[1] = estrdup(EARGF(usage()));
		break;
	case 'l':
