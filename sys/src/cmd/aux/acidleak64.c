...
int
markblock(uintptr from, uintptr fromval, Block *b)
{
	Data *d;
	uintptr top;
	Block *nb;
...
static char fmt64[] = "block %#.16p %#.16p %#.16p %#.16p %s %s\n";
static char fmt32[] = "block %#.8p %#.8p %#.8p %#.8p %s %s\n";
...
main(int argc, char **argv)
{
	...
	uintptr allocstart, allocend, len, u;
	Data *d, *ed;
	Block *b, *eb;
	...
	while(p=Brdline(&bio, '\n')) {
		...
		if(nf >= 4 && strcmp(f[0], "data") == 0) {
			...
			data[ndata].addr = strtoull(f[1], nil, 0);
			data[ndata].val = strtoull(f[2], nil, 0);
		...
		if(nf >= 5 && (strcmp(f[0], "block") == 0 || strcmp(f[0], "free") == 0)) {
			...
			block[nblock].addr = strtoull(f[1], nil, 0);
			block[nblock].size = strtoull(f[2], nil, 0);
			block[nblock].w0 = strtoull(f[3], nil, 0);
			block[nblock].w1 = strtoull(f[4], nil, 0);
			if (nf >= 7) {
		...
		if(nf >= 4 && strcmp(f[0], "range") == 0 && strcmp(f[1], "alloc") == 0) {
			allocstart = strtoull(f[2], 0, 0)&~15;
			allocend = strtoull(f[3], 0, 0);
		}
	}
...
//fprint(2, "\t%d\n", c);
		...
		/*
		 * this big of terrible code gets around the fact that #p,
		 * though it knows the size of uintptr, doesn't pad to
		 * the proper size.  we have to supply .8 or .16 ourselves.
		 */
		...
		for(b=block; b<eb; b++)
			if(b->mark == 0 && !b->free)
				Bprint(&bio, p, b->addr, b->size, b->w0, b->w1, b->s0, b->s1);
	}
	Bterm(&bio);
	exits("");
}
