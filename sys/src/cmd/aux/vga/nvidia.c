...
snarf(Vga* vga, Ctlr* ctlr)
{
	...
	if(vga->private == nil){
		...
		case 0x0210:	/* GeForce 6200 */
		case 0x0220:
		case 0x0290:	/* GeForce 7900 */
		case 0x0390:	/* GeForce 6100 */
		case 0x03D0:	/* ? */
		case 0x0240:	/* nForce 4 6100/6150 */
			nv->arch = 40;
			break;
		default:
     			break;
	...
	nv->arb[0] = vgaxi(Crtx, 0x1B);
	nv->arb[1] = vgaxi(Crtx, 0x20);
	if(nv->arch >= 30)
		nv->arb[1] |= vgaxi(Crtx, 0x47) << 8;	// xf86 uses 1 bit, but must be bug since it uses 0x480
...
/* calculations in KHz to avoid overflow */
enum{
	KHz = 1000,
};

static void
clock1stage(ulong vclk, Vga* vga, Nvidia *nv)
{
	ulong m0, ν, Δ, Δ0, f, m, n, p;
    
	Δ0 = ~0;
	vclk /= KHz;
	ν = nv->crystalfreq/KHz;
	...
	for(p = 0; p <= vga->p[1]; p++){
		f = vclk<<p;
		if(f < 128000 || f > 350000)
			continue;
		for(m = m0; m <= vga->m[1]; m++){
			n = ((vclk<<p)*m)/ν;
			if(n > 255)
				continue;
			f = ν*n/m>>p;
...
print("bzzt!\n");
			...
			vga->d[0] = Δ;
			Δ0 = Δ;
			print("n = %uld; m = %uld; p = %uld; d = %uld; %uld (%uld)\n", n, m, p, Δ, vga->f[0], vclk);
            		}
	}
 }

static void
clock2stage(ulong vclk, Vga* vga, Nvidia *nv)
{
	ulong ν, Δ, Δ0, f, m, n, p;

	Δ0 = ~0;
	vclk /= KHz;
	ν = nv->crystalfreq/KHz<<2;
	vga->p[1] = 6;
	vga->m[1] = 13;
	vga->f[1] = 400000*KHz<<2;

	for(p = 0; p <= vga->p[1]; p++){
		f = vclk<<p;
		if(f < 400000 || f > 1000000)
			continue;
		for(m = 1; m <= vga->m[1]; m++){
			n = ((vclk<<p)*m)/ν;
			if(n < 5 || n > 255)
			...
			vga->d[0] = Δ;
			Δ0 = Δ;
			print("n = %uld; m = %uld; p = %uld; d = %uld; %uld (%uld)\n", n, m, p, Δ, nv->crystalfreq*n/(m<<p), vclk);
		}
	}
}
...
clock(Vga* vga, Ctlr* ctlr)
{
	...
	if (nv->twostagepll) {
		nv->vpllB = 0x80000401;
		clock2stage(vga->f[0], vga, nv);
	} else
		clock1stage(vga->f[0], vga, nv);

if(getenv("doit") == nil)exits("fu");
	if (vga->d[0] > vga->f[0])
		error("%s: vclk %lud/%uld out of range\n", ctlr->name, vga->f[0], vga->d[0]);
}
...
arb(Vga *vga, Ctlr *)
{
	...
	nv = vga->private;
	if((nv->did&0xfff0) == 0x240){
print("setting 0240 arb\n");
		nv->arb[0] = 256;
		nv->arb[1] = 0x480;
	}
}
...
init(Vga* vga, Ctlr* ctlr)
{
	...
	int tmp, pixeldepth;
	ulong cursorstart;
	int 	hdis, hsta, hend, htot, hbls, hble;
	int	vdis, vsta, vtot, vbls, vble;
	...
	nv->usablesz = vga->vmz-128*1024;
	if(nv->arch >= 40)
		nv->usablesz = vga->vmz-560*1024;
	...
	if(nv->arch == 4) {
		...
		cursorstart = nv->usablesz+32*1024;
...
//	USED(hdis, hsta, hend, htot, hbls, hble);
//	USED(vdis, vsta, vtot, vbls);

	vga->crt[0x03] = 0x80 | (htot+1 & 0x1F);
	vga->crt[0x04] = hsta;				// prevents r side flashes
	vga->crt[0x06] = vtot;
	vga->crt[0x12] = vdis;
...
//	vga->crt[0x17] |= 8;
//	vga->crt[0x17] &= ~ 0x40;
/*	NB: nv driver has this

	vga->attribute[0x10] = 1;
	if(nv->television)
...
//		vga->crt[0x05] &= ~0x80;
	...
	nv->pixel = pixeldepth = (mode->z +1)/8;
	if (pixeldepth > 2)
		nv->pixel = 3;
	else
		nv->pixel = pixeldepth;
	nv->repaint0 = ((mode->x/8*pixeldepth) & 0x0700) >> 3;
...
nv->pcrtc[0x800/4] = 0;
	...
	if (nv->arch >= 40) {
		...
		nv->pramin[0x084E] = nv->usablesz | 0x00000002;
	} else {
		nv->pramin[0x0000] = 0x80000010;
		...
		nv->pramin[0x0826] = nv->usablesz | 0x00000002;
	...
	vgaxo(Crtx, 0x1B, nv->arb[0]);
	vgaxo(Crtx, 0x20, nv->arb[1]);
	if(nv->arch >= 30)
		vgaxo(Crtx, 0x47, nv->arb[1]>>8);
	// mode->fifo
	vgaxo(Crtx, 0x1C, vgaxi(Crtx, 0x1C) & ~(1<<5));
...
dump(Vga* vga, Ctlr* ctlr)
{
	...
	printitem(ctlr->name, "arb[0]");
	Bprint(&stdout, " %lux\n", nv->arb[0]);
	printitem(ctlr->name, "arb[1]");
	Bprint(&stdout, " %lux\n", nv->arb[1]);
