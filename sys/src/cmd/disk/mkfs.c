...
main(int argc, char **argv)
{
	...
	case 'D':
		...
		break;
	case 'k':
		modemask = DMDIR|DMAPPEND|DMEXCL|0777;
		break;
	case 'n':
		name = EARGF(usage());
		break;
...
copy(Dir *d)
{
	...
	if(badchar && strpbrk(oldfile, "\t \n\r")){
		warn("bogus filename %q", oldfile);
		return;
	}
...
mkdir(Dir *d)
{
	...
	fd = create(newfile, OREAD, d->mode & modemask);
	nulldir(&nd);
	nd.mode = d->mode & modemask;
	nd.gid = d->gid;
	nd.mtime = d->mtime;
...
loop:
	...
	f->old = nil;
	getname(p, &f->old);
	if(f->old != nil && (f->old[0] == 0 || strcmp(f->old, "-") == 0)){
		free(f->old);
		f->old = nil;
	}
	setnames(f);
...
usage(void)
{
	fprint(2, "usage: %q [-abkprvx] [-d root] [-n name] [-s source] [-u users] [-z n] proto ...\n", prog);
	exits("usage");
}
