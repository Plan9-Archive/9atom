...
Face*
dirface0(char *dir, char *num, Face *f)
{
	char *from, *date;
	...
	if(f->str[Ssubject] == nil)
		f->str[Ssubject] = estrdup(iline(p, &p));	/* subject */
...
{
	return dirface0(dir, num, 0);
}

Face*
pathface(char *path, Face *f)
{
	char buf[1024], *t;
