...
static struct {
	...
	23,	"4-bit ISDN μ-law ITU-T G.721 ADPCM compressed",
...
isau(void)
{
	...
	if(getbe(buf, 4) != 0x2e736e64)
		return 0;
	w = getbe(buf+3*4, 4);
	rate = getbe(buf+4*4, 4);
	nchan = getbe(buf+5*4, 4);
	e = 0;
	for(i = 0; i < nelem(autab); i++)
		if(w == autab[i].i)
			e = i;
	if(mime)
		print("sound/basic\n");
	else
		print("au file %dHz %d chan %s\n", rate, nchan, autab[e].s);
	return 1;
}
