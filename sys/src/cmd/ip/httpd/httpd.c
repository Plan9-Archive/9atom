...
static	uchar 		*certificate;
static	int		certlen;
static	PEMChain	*certchain;	
static	int		tflag;

#define	redirdbg(...)	if(tflag)logit(__VA_ARGS__); else USED(certlen)
...
top:
	...
	if(once == 0){
		/* delayedbecause magic can't parse c->header, cf: hgethead() */
		if(hparseheaders(c, 15*60*1000) < 0)
			exits("failed");
		once = 1;
	}
	if(origuri[0]=='/' && origuri[1]=='~'){
		redirdbg(c, "%d: ~ %s", nredirect, origuri);
		n = strlen(origuri) + 4;
		newpath = halloc(c, n);
		snprint(newpath, n, "/who/%s", origuri+2);
		c->req.uri = uri = newpath;
		goto top;
	}
	snprint(virtualhost, sizeof virtualhost, "http://%s%s", c->head.host, origuri);
	...
	if(newuri){
		if(isdecorated(newuri)){
			...
			n = hparseuri(c, undecorated(newuri));
			if(n != 1)
				return n;
			uri = c->req.uri;
