...
int
crackpath(char *s, char *page, int npage, char *sect, int nsect)
{
	char *p;
	...
	*sect = 0;
	...
	if(p == nil)
		return -1;
	snprint(page, npage, "%s", p+1);
	snprint(sect, nsect, "%.*s", (int)(p-s), s);
	return 0;
}
...
searchfor(char *search)
{
	...
	char *p, *sp;
	Biobufhdr *b;
	char *arg[32], page[32], sect[32];

	hprint(hout, "<!DOCTYPE html PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\">\n");
	hprint(hout, "<html>\n");
	hprint(hout, "<link rel=StyleSheet href=\"/manstyle.css\">\n");
	hprint(hout, "<head><title>plan 9 search for %H</title>\n", search);
	...
	hprint(hout, "<INPUT TYPE=\"submit\" VALUE=\"search\">\n");
	hprint(hout, "</FORM>\n");
	...
	for(;;){
		...
		if(sp == nil)
			continue;
		sp++;
		crackpath(sp, page, sizeof page, sect, sizeof sect);
		hprint(hout, "<p><a href=\"/magic/man2html/%U\"><i>%H(%H)</a>\n",
			sp, page, sect);
	}
	hprint(hout, "</body>");
