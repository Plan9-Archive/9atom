...
checkreq(HConnect *c, HContent *type, HContent *enc, long mtime, char *etag)
{
	...
	/*
	 * this is broken with modern browsers because they interpret
	 * content-encoding to mean transfer encoding.  since we don't
	 * offer alternate encodings, we'll just drop this check.
