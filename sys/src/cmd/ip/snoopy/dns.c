...
" type=%d alg=%d labels=%d ttl=%lud exp=%lud incep=%lud tag=%d signer=%s data=%.*H",
	...
	case Tcert:
		...
		break;
	case Topt:
		m->p = seprint(m->p, m->e, " edns0 udpsz=%d", rr->owner->class);
		break;
	}
	rrfree(rr);
}
...
p_seprint(Msg *m)
{
	...
	if((e = convM2DNS(m->ps, m->pe-m->ps, &dm, &rv)) != nil){
		m->p = seprint(m->p, m->e, "error: %s", e);
		return 0;
	}
	rsp = "??";
	if(rv >= 0 && rv < nelem(rsptab) && rsptab[rv])
		rsp = rsptab[rv];
	m->p = seprint(m->p, m->e, "id=%d flags=%#ux %s", dm.id, dm.flags, rsp);
	m->p = seprint(m->p, m->e, " %d/%d/", dm.qdcount, dm.ancount);
	m->p = seprint(m->p, m->e, "%d/%d", dm.nscount, dm.arcount);
	donext(m);
	return 0;
}
