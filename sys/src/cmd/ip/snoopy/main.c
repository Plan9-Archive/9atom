 ...
 * pcap does endian conversion in the reader by magic sniffing,
...
struct pcap_file_header {
	...
	u32int		thiszone;    /* gmt to local correction */
...
struct pcap_pkthdr {
	u32int	tssec;	/* time stamp seconds */
	u32int	tsusec;	/* time stamp microseconds */
...
tracepkt(uchar *ps, int pktlen)
{
	...
	if(pcap){
		...
		goo->tssec = pkttime/1000000000;
		goo->tsusec = pkttime%1000000000/1000;
		goo->caplen = len;
		goo->len = pktlen;
		Bwrite(&out, goo, len+16);
	} else {
		hnputs(ps-10, len);
		hnputl(ps-8, pkttime>>32);
		hnputl(ps-4, pkttime);
		Bwrite(&out, ps-10, len+10);
	}
}
