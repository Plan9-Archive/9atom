...
eresized(int new)
{
	...
	if(new && getwindow(display, Refnone) < 0){
		fprint(2, "wbmp: can't reattach to window\n");
		exits("resize");
	}
	if(image == nil)
		return;
...
main(int argc, char *argv[])
{
	...
	case '9':		/* produce plan 9, uncompressed, bitmap file; no display by default */
		...
		break;
	default:
		fprint(2, "usage: wbmp -39cdektv  [file.wbmp ...]\n");
		exits("usage");
	}ARGEND;
	...
	if(argc == 0)
		...
		for(i=0; i<argc; i++){
			...
			if((nineflag || cflag) && argc>1 && err==nil){
				fprint(2, "wbmp: exiting after one file\n");
				break;
			}
		}
	}
...
init(void)
{
	...
	if(inited == 0){
		if(initdraw(0, 0, 0) < 0){
			fprint(2, "wbmp: initdraw failed: %r");
			return -1;
		}
...
show(int fd, char *name)
{
	...
	if(array == nil || array[0]==nil){
		fprint(2, "wbmp: decode %s failed: %r\n", name);
		return "decode";
	}
	if(!dflag){
		if(init() < 0)
	...
	if(c == nil){
		fprint(2, "wbmp: converting %s to local format failed: %r\n", name);
		return "torgbv";
	}
	if(!dflag){
		if(r->chandesc == CY)
		...
		if(i == nil){
			fprint(2, "wbmp: allocimage %s failed: %r\n", name);
			return "allocimage";
		}
		if(loadimage(i, i->r, c->chans[0], c->chanlen) < 0){
			fprint(2, "wbmp: loadimage %s failed: %r\n", name);
			return "loadimage";
		}
		image = i;
		eresized(0);
