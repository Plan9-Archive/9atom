...
retry:
	...
	switch(type){
	default:
		/* unknown type, just ignore it (will likely invalidate the query) */
		dnslog("convM2DNS: unknown type %d", type);
	case Tall:		/* ignore (should return an error) */
		sp->p = data + len;
		rrfree(rp);
		goto retry;
	case Thinfo:
...
rrloop(Scan *sp, char *what, int count, int quest)
{
	...
	if(i != count)
	if(strcmp(what, "hints") != 0 && i != 0 && count != 1){
		dnslog("rrloop: got %d of %d %s", i, count, what);
		for(rp = first; rp != nil; rp = rp->next)
			dnslog("  %R", rp);
	}
//	if(first)
...
convM2DNS(uchar *buf, int len, DNSmsg *m, int *codep)
{
	...
	if (sp->stop)
		sp->rcode = Rok;
if(sp->trunc || m->flags & Ftrunc)dnslog("len %d trunc %.4ux flags %.4ux", len, sp->trunc, m->flags);
	if (codep)
		*codep = sp->rcode;
	return err;
}
