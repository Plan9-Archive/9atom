...
Ndbtuple*	ellookup(Network*, char*, char*, int);
char*		eltrans(Ndbtuple*, Network*, char*, char*, int);
...
[Nel]		{ "el",	ellookup,	eltrans,	1, 0 },
[Nssh]	{ "ssh",	iplookup,	iptrans,	1, 0 },
		{ 0 },
};
...
usage(void)
{
	fprint(2, "usage: %s [-dn] [-S srv] [-f ndb-file] [-x netmtpt]\n", argv0);
	exits("usage");
}
...
main(int argc, char *argv[])
{
	...
	case 'x':
		...
		break;
	case 'S':
		snprint(servefile, sizeof(servefile), "#s/%s", EARGF(usage()));
		break;
	}ARGEND
	if(argc != 0)
		usage();
...
elattr(char *name)
{
	...
	if(strncmp(name, "5100", 4) == 0 && strspn(name + 4, hex) == 12)
		return "el";
	n = strtoul(name, &p, 0);
	if(n <= 0xffff && *p == 0)
		return "shelf";
...
elid(void)
{
	...
	if(myelname != nil)
		return;
	if(myelname == 0 && (p = getenv("elname")) != nil && *p != 0){
		attr = elattr(p);
		if(strcmp(attr, "el") != 0)
			myelname = strdup(p);
	}
	if(myelname == 0 && netdb != nil){
		ndbreopen(netdb);
		for(tt = t = ndbparse(netdb); t != nil; t = t->entry){
			if(strcmp(t->attr, "el") == 0){
				mysysname = strdup(t->val);
				break;
			}
		}
	...
	if(myelname == 0){
		snprint(buf, sizeof buf, "#l0/ether0/addr");
		fd = open(buf, OREAD);
		if(fd != -1 && read(fd, buf, 12) == 12){
			buf[12] = 0;
			myelname = smprint("5100%s", buf);
		}
		close(fd);
	}
	if(myelname != nil){
		fd = open("/net/el/ctl", OWRITE);
		if(fd != -1){
			fprint(fd, "add %s", myelname);
			fprint(fd, "bind ether #l0/ether0");
			if(mysysname != nil)
				fprint(fd, "arpattr add sysname %s", mysysname); 	
			close(fd);
		}
	}
}
...
lookup(Mfile *mf)
{
	...
	if(db == 0)
		ndbinit();
	if(db == 0){
		syslog(0, "cs", "can't open mf->network database");
		return -1;
	}
...
/*
 *  translate an el service name into a port number.  If it's a numeric port
 *  number, look for restricted access.
 *
 *  the service '*' needs no translation.
 */
char*
elserv(Network *np, char *name, char *buf, int blen)
{
	char *p, port[10];
	int n, alpha = 0;
	Rune r;
	...
	for(p = name; *p; p += n){
		n = chartorune(&r, p);
		if(n < Runeself && isdigit(r))
			{}
		else if(isalpharune(r) || r == '-' || r == '$')
			alpha = 1;
		else
			return 0;
	}
...
elarpscan(char *host)
{
	...
	for(; (s = Brdstr(b, '\n', 1)) && t == nil; free(s)){
		...
		if(n <= 0)
			continue;
		m = getfields(f[n-1], g, nelem(g), 1, " =");
		for(i = 0; i < m-1; i += 2)
			if(strcmp(g[i+0], "sysname") == 0)
			if(strcmp(g[i+1], host) == 0){
				t = ndbnew("el", f[1]);
				break;
			}
	}
...
/*
 *  lookup (and translate) an el destination
 */
Ndbtuple*
ellookup(Network *np, char *host, char *serv, int)
{
	char *attr, *q, *dnsname, ts[Maxservice];
	Ndbs s;
	Ndbtuple *t;
	 ...
	 */
	werrstr("can't translate address");
	if(serv==0 || elserv(np, serv, ts, sizeof ts) == 0){
		werrstr("can't translate service");
		return 0;
	}
	...
	attr = elattr(host);
	if(strcmp(attr, "el") == 0)
		return ndbnew("el", host);

	/*
	 * check the el arp table
	 */
	if(t = elarpscan(host))
		return t;
	...
	if(strcmp(attr, "dom") == 0)
		t = dnsiplookup(host, &s);
	if(t == 0)
		free(ndbgetvalue(db, &s, attr, host, "el", &t));
	if(t == 0){
		q = ndbgetvalue(db, &s, attr, host, "ether", &t);
		ndbfree(t);
		if(q != nil){
			t = ndbnew("el", smprint("5100%s", q));
			free(q);
		}
	}
	if(t == 0){
	...
	if(t == 0)
		return 0;
	for(Ndbtuple *nt = t; nt != nil; nt = nt->entry){
...
/*
 *  translate an el address
 */
char*
eltrans(Ndbtuple *t, Network *np, char *serv, char *rem, int hack)
{
	char ts[Maxservice], reply[Maxreply], x[Maxservice];
