...
static RR*	spfrr(Ndbtuple*, Ndbtuple*);
...
{
	return txtlikerr(rralloc(Ttxt), pair);
}
static RR*
spfrr(Ndbtuple*, Ndbtuple *pair)
{
	return txtlikerr(rralloc(Tspf), pair);
}
static RR*
...
dbpair2cache(DN *dp, Ndbtuple *entry, Ndbtuple *pair)
{
	...
	if(cistrcmp(pair->attr, "ip") == 0 ||
	   cistrcmp(pair->attr, "ipv6") == 0){
		...
		rp = txtrr(entry, pair);
	else if(cistrcmp(pair->attr, "spfrr") == 0)
		rp = spfrr(entry, pair);
	if(rp == nil)
		return;
...
int
outsidens(int n, uchar ip[IPaddrlen])
{
	int i;
	Ndbtuple *t;
	...
	for (t = outnmsrvs; t != nil; t = t->entry)
		if(strcmp(t->attr, "ip") == 0 && parseip(ip, t->val) != -1 && i++ == n)
			return 0;
	return -1;
}
