...
usage(void)
{
	fprint(2, "%s: [-erx] [-f db-file] [[@server] domain [type]]\n", argv0);
	exits("usage");
}
...
void
logreply(char *net, int id, uchar *addr, DNSmsg *mp)
{
	RR *rp;
	char buf[12], resp[32];

	print("%d: rcvd %s from %s %I (%s%s%s%s%s) %.2ux\n",
		id, respname(mp->flags & Rmask, resp, sizeof resp), net, addr,
		mp->flags & Fauth? "authoritative": "",
		mp->flags & Ftrunc? " truncated": "",
		mp->flags & Frecurse? " recurse": "",
		mp->flags & Fcanrec? " can_recurse": "",
		(mp->flags & (Fauth|Rmask)) == (Fauth|Rname)? " nx": "", mp->flags);
...
void
logsend(char *proto, int id, int subid, uchar *addr, char *sname, char *rname, int type)
{
	char buf[12];

	print("%d.%d: sending to %s %I/%s %s %s\n", id, subid,
		proto, addr, sname, rname, rrname(type, buf, sizeof buf));
}

void
logtrunc(char *oldproto, char *newproto, int id, int subid, uchar *addr, char *sname, char *rname, int type)
{
	char buf[12];

	USED(addr, sname);
	print("%d.%d: %s: truncated retry %s: question: %s %s\n", id, subid,
		oldproto, newproto, rname, rrname(type, buf, sizeof buf));
}
