...
emalloc(long n)
{
	...
	v = (void*)malloc(n);
	if(v == 0){
//		sysfatal("malloc: %r from %p %ld\n", getcallerpc(&n), n);
		fprintf(stderr, "malloc: %r for %ld bytes\n", n);
		abort();
	}
...
readmap(void)
{
	...
	if(*mapname != '/'){
		path = emalloc(strlen(hostfontdir) + strlen(mapname) +
			strlen(suffix) + 2);
	...
	if((fd = open(path, 0))!= -1){
		...
		stringspace = emalloc(sbuf.st_size + 2);
...
copyfonts(char *list)
{
	...
	while((font = strtok(NULL, " \t\n"))!= NULL){
		...
		if((n = lookup(font))< next){
			if(*map[n].file != '/'){
				path = emalloc(strlen(hostfontdir)+strlen(map[n].file)+2);
