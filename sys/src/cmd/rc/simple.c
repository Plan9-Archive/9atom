...
execdot(void)
{
	...
	for(path = searchpath(zero); path; path = path->next){
		...
		fd = open(file, 0);
		stdin = strcmp(file, "/dev/stdin") == 0;
		free(file);
		if(fd >= 0)
			break;
		if(stdin){		/* for sun & ucb */
			fd = Dup1(0);
			if(fd>=0)
				break;
		}
...
void
execprintenv(void){	/* mildly wrong -- should fork before writing */
	struct io out[1];
	var *v;
	word *a;
	char *s;
