...
long
SRlogsense(ScsiReq *rp, uchar arg, uchar page, ushort pp, uchar *list, long nbytes)
{
	uchar cmd[10];
...
SRmodeselect10(ScsiReq *rp, uchar *list, long nbytes)
{
	...
	memset(cmd, 0, sizeof cmd);
	if((rp->flags & Finqok) && (rp->inquiry[2] & 0x07) >= 2)
		cmd[1] = 0x10;	/* standard (scsi-2) format */
	cmd[0] = ScmdMselect10;
	cmd[7] = nbytes>>8;
	cmd[8] = nbytes;
	rp->cmd.p = cmd;
...
long
SRreadattr(ScsiReq *rp, uchar sa, uchar *list, long nbytes)
{
	uchar cmd[16];
...
long
SRmediaserial(ScsiReq *r, uchar *c, ulong l)
{
	uchar cmd[12];
...
long
SRvpd(ScsiReq *rp, int page, uchar *data, long l)
{
	uchar cmd[6];
	...
	*status = atoi(buf);
	if(n < 0 && (exabyte || *status != STcheck)){
		...
		for(i = 0; i < nelem(errtab); i++)
			if(errtab[i].k == *status)
				p = errtab[i].s;
		fprint(2, "scsireq: %s: status %d %s: data transfer: %r\n",
			path, *status, p);
	}
	return n;
}
