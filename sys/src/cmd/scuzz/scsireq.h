...
enum {					/* SCSI command codes */
	...
	ScmdReadattr	= 0x8c,		/* read attribute */
...
#define PUTBELONG(p, ul) ((p)[0] = (ul)>>24, (p)[1] = (ul)>>16, \
			  (p)[2] = (ul)>>8,  (p)[3] = (ul))
#define GETBE16(p)	((ushort)(p)[1]<<8 | (p)[2])
#define GETBE24(p)	((ulong)(p)[0]<<16 | (p)[1]<<8 | (p)[2])
#define PUTBE24(p, ul)	((p)[0] = (ul)>>16, (p)[1] = (ul)>>8, (p)[2] = (ul))
#define GETBEVL(p) ((uvlong)GETBELONG(p)<<32 | GETBELONG(p + 4))
#define PUTBEVL(p, u) ((uvlong)PUTBELONG(p, u>>32) | GETBELONG(p + 4, u))
...
long	SRlogsense(ScsiReq*, uchar, uchar, ushort, uchar*, long);
...
long	SRreadattr(ScsiReq*, uchar, uchar*, long);
long	SRstart(ScsiReq*, uchar);
long	SRrcapacity(ScsiReq*, uchar*);
long	SRrcapacity16(ScsiReq*, uchar*);
long	SRmediaserial(ScsiReq*, uchar*, ulong);
long	SRvpd(ScsiReq*, int, uchar*, long);
