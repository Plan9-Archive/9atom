...
for(i = 0; i < 16; ++i) fprint(2, "%02x", c->s2cek[i]);
fprint(2, "\nc2sek: ");
for(i = 0; i < 16; ++i) fprint(2, "%02x", c->c2sek[i]);
fprint(2, "\ns2civ: ");
for(i = 0; i < 8; ++i) fprint(2, "%02x", c->s2civ[i]);
fprint(2, "\nc2siv: ");
for(i = 0; i < 8; ++i) fprint(2, "%02x", c->c2siv[i]);
fprint(2, "\n");

	cs = emalloc9p(sizeof(CipherState));
memset(cs, '\0', sizeof(CipherState));
fprint(2,"cs:%p\n", cs);
	if(dir)
