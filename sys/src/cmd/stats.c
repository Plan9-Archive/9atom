...
#define	MAXNUM	14	/* maximum number of numbers on data line */
...
struct Machine
{
	...
	int		nproc;
	int		lgproc;
	uvlong		netetherstats[Addr + 1];
	uvlong		prevetherstats[Addr + 1];
	uvlong		netether1stats[Addr + 1];
	uvlong		prevether1stats[Addr + 1];
...
void	contextval(Machine*, uvlong*, uvlong*, int),
	...
	ether1val(Machine*, uvlong*, uvlong*, int),
	ether1errval(Machine*, uvlong*, uvlong*, int),
	ether1inval(Machine*, uvlong*, uvlong*, int),
	ether1outval(Machine*, uvlong*, uvlong*, int),
...
Font	*mediumfont;
char	*mysysname;
char	argchars[] = "128bceEfiImlnNpstwz";
int	pids[NPROC];
...
loadbuf(Machine *m, int *fd)
{
	...
	if(*fd < 0)
		return 0;
	n = pread(*fd, m->buf, sizeof m->buf-1, 0);
	if(n <= 0){
		close(*fd);
		*fd = -1;
		return 0;
...
initmach(Machine *m, char *name)
{
	...
	snprint(buf, sizeof buf, "%s/net.alt/ether1/stats", mpt);
	m->ether1fd = open(buf, OREAD);
	if(loadbuf(m, &m->ether1fd) && readnums(m, nelem(m->netether1stats), a, 1))
		memmove(m->netether1stats, a, sizeof m->netether1stats);
...
needether1(int init)
{
	return init | present[Mether1] | present[Mether1in] | present[Mether1out] | present[Mether1err];
}
...
readmach(Machine *m, int init)
{
	...
	if(m->remote && (m->disable || setjmp(catchalarm))){
		...
		memmove(m->netether1stats, m->prevether1stats, sizeof m->netether1stats);
		return;
	}
	...
	if(needether1(init) && loadbuf(m, &m->ether1fd) && readnums(m, nelem(m->netether1stats), a, 1)){
		memmove(m->prevether1stats, m->netether1stats, sizeof m->netether1stats);
		memmove(m->netether1stats, a, sizeof m->netether1stats);
	}
...
void
ethermax(Machine*, int, uvlong *a, uvlong *vmax)
{
	uvlong s;
	...
	if(s <= 10)
		s = 50;
	*vmax = (s*sleeptime)/1000;
	if(*vmax < 1 || *vmax > 100000){
		fprint(2, "vmax out of range %lld %lld\n", *vmax, a[Mbps]);
		*vmax = 1000;
	}
}
...
etherval(Machine *m, uvlong *v, uvlong *vmax, int init)
{
	*v = m->netetherstats[In]-m->prevetherstats[In] + m->netetherstats[Out]-m->prevetherstats[Out];
	ethermax(m, init, m->netetherstats, vmax);
}
...
void
ether1val(Machine *m, uvlong *v, uvlong *vmax, int init)
{
	*v = m->netether1stats[In]-m->prevether1stats[In] + m->netether1stats[Out]-m->prevether1stats[Out];
	ethermax(m, init, m->netether1stats, vmax);
}

void
ether1inval(Machine *m, uvlong *v, uvlong *vmax, int init)
{
	*v = m->netether1stats[In]-m->prevether1stats[In];
	ethermax(m, init, m->netether1stats, vmax);
}

void
ether1outval(Machine *m, uvlong *v, uvlong *vmax, int init)
{
	*v = m->netether1stats[Out]-m->prevether1stats[Out];
	ethermax(m, init, m->netether1stats, vmax);
}

void
ether1errval(Machine *m, uvlong *v, uvlong *vmax, int init)
{
	int i;

	*v = 0;
	for(i=Err0; i<nelem(m->netether1stats); i++)
		*v += m->netether1stats[i];
	i = sleeptime/100;
	if(i == 0)
		i = 1;
	*vmax = i*m->nproc;
...
main(int argc, char *argv[])
{
	...
	for(;;){
		...
		if(sleeptime != st0 && δ < sleeptime/32)
		if(prescale >= 2 && sleeptime >= 2){
			prescale /= 2;
			sleeptime /= 2;
		}
		δ = (nsec() - t0)/1000000;
