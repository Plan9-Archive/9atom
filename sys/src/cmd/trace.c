...
#pragma	varargck	type	"t"		u64int
#pragma	varargck	type	"U"		u64int
...
//print("newestts %t, period %t, %d-%d\n", newestts, period, time2x(oldestts), time2x(newestts));
	...
	for (n = 0; n != ntasks; n++) {
		...
		if(t->tevents[SRelease])
			snprint(buf, sizeof(buf), " per %t — avg: %t max: %t",
				(u64int)(s/t->tevents[SRelease]),
				(u64int)(t->runtime/t->tevents[SRelease]),
				(u64int)t->runmax);
			...
			snprint(buf, sizeof(buf), " total: %t", (u64int)t->total);
	...
	ss = 0;
	while(x < Width){
		snprint(buf, sizeof(buf), "%t", (u64int)ss);
	...
	snprint(buf, sizeof(buf), "%t", (u64int)now);
...
timeconv(Fmt *f)
{
	...
	if (t > S(1)){
		t += OneRound;
		snprint(buf, sizeof buf, "%s%d.%.3ds", sign, (int)(t / S(1)), (int)(t % S(1))/1000000);
	}else if (t > MS(1)){
		t += MilliRound;
		snprint(buf, sizeof buf, "%s%d.%.3dms", sign, (int)(t / MS(1)), (int)(t % MS(1))/1000);
	}else if (t > US(1))
		snprint(buf, sizeof buf, "%s%d.%.3dµs", sign, (int)(t / US(1)), (int)(t % US(1)));
	else
		snprint(buf, sizeof buf, "%s%dns", sign, (int)t);
