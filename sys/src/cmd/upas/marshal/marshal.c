...
usage(void)
{
	fprint(2, "usage: %s [-Fn] [-drx#] [-s subject] [-C cc] [-B bcc] [-t type]"
...
{
	char *subject, *type, *boundary, *sentfolder;
	char file[Pathlen], *ccargv[32], *bccargv[32];
	int bccargc, ccargc, flags, fd, noinput, headersrv;
	Addr *bcc, *cc, *to;
	Attach *first, **l, *a;
	Biobuf in, out, *b;
	String *hdrstring;
	...
	mailfmtinstall();		/* %U is rfc2047 fmt */
	...
	sentfolder = getenv("sentfolder");
	if(sentfolder == nil && Fflag)
		sentfolder = argv[0];
	fd = sendmail(to, cc, bcc, &pid, sentfolder);
	if(fd < 0)
...
msgiddom(void)
{
	...
	if(p == 0)
		...
		p++;
	snprint(buf, sizeof buf, "%s.%s", h, p);
	return buf;
}
...
printmessageid(Biobuf *b)
{
	...
	id[2*i] = 0;
	/*
	 * botch: should be the same as ../smtp/smtp.c:hostdomain
	 */
	return Bprint(b, "Message-ID: <%s@%s>\n", id, msgiddom());
}
...
{
	int fd, n, c;
	char buf[256], ibuf[256], *s;
	Biobuf *b;

	/* tell upas/fs we've replied to this */
	snprint(buf, sizeof buf, "%s/flags", dir);
	fd = open(buf, OWRITE);
	write(fd, "a", 1);
	close(fd);

	snprint(buf, sizeof buf, "%s/messageid", dir);
	fd = open(buf, OREAD);
	if(fd < 0)
		return 0;
	...
	if(n <= 0)
		return 0;
	ibuf[n] = 0;
	Bprint(out, "In-Reply-To: <%s>\n", ibuf);
	snprint(buf, sizeof buf, "%s/references", dir);
	Bprint(out, "References:");
	c = ' ';
	if(b = Bopen(buf, OREAD)){
		while(s = Brdstr(b, '\n', 1)){
			Bprint(out, "%c<%s>\n", c, s);
			c = '\t';
			free(s);
...
mksubject(char *line)
{
	...
	snprint(buf, sizeof buf, "Subject: %U", strchr(line, ':') + 1);
	return buf;
}
