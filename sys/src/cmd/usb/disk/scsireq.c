...
SRread(ScsiReq *rp, void *buf, long nbytes)
{
	...
	if(rp->lbsize == 0 || (nbytes % rp->lbsize)){
		if(diskdebug)
			if (nbytes % rp->lbsize)
...
mkcode(ScsiReq *r)
{
	return scsierror(r->sense[12], r->sense[13]);
}
...
SRrecover(ScsiReq *rp)
{
	...
	default:
		fprint(2, "usb/disk: %s: %.6ux %s\n", s, mkascq(rp), mkcode(rp));
		break;
	case 0x0402:
