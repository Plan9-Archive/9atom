...
rtl8139attach(Ether* edev)
{
	...
	if(ctlr->alloc == nil){
		...
		ctlr->alloc = ialloc(ctlr->rblen+16 + Ntd*Tdbsz + 32, 0);
		rtl8139init(edev);
	}
	qunlock(&ctlr->alock);
}
...
rtl8139txstart(Ether* edev)
{
	...
	while(ctlr->ntd < Ntd){
		...
		if(((int)bp->data) & 0x03){
			...
			td->bp = bp;
			csr32w(ctlr, td->tsad, PCIWADDR(bp->data));
			ctlr->taligned++;
		}
...
rtl8139receive(Ether* edev)
{
	...
	while(!(csr8r(ctlr, Cr) & Bufe)){
		...
		if((bp = mballoc(length, 0, Mbeth1)) != nil){
			bp->count = 0;
			if(p+length >= ctlr->rbstart+ctlr->rblen){
				l = ctlr->rbstart+ctlr->rblen - p;
				memmove(bp->data+bp->count, p, l);
				bp->count += l;
				length -= l;
				p = ctlr->rbstart;
			}
			if(length > 0){
				memmove(bp->data+bp->count, p, length);
				bp->count += length;
			}
			bp->count -= 4;
			etheriq(edev, bp);
