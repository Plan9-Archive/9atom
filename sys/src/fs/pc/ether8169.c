...
enum {					/* Tcr */
	...
	Macv19a		= 0x2c000000,	/* RTL8111e; guesswork */
...
rtl8169replenish(Ctlr* ctlr)
{
	...
	while(NEXT(rdt, ctlr->nrd) != ctlr->rdh){
		...
		if(ctlr->rb[rdt] == nil){
			 ...
			 */
			bp = mballoc(Mps, 0, Mbeth1);
			if(bp == nil){
				print("no available buffers\n");
				break;
			}
			ctlr->rb[rdt] = bp;
			d->addrlo = PCIWADDR(bp->data);
			d->addrhi = 0;
		}
		coherence();
		d->control |= Own|Mps;
...
rtl8169attach(Ether* edev)
{
	...
	if(ctlr->init == 0){
		 ...
		 */
		ctlr->td = ialloc(sizeof(D)*Ntd, 256);
		ctlr->tb = ialloc(Ntd*sizeof(Msgbuf*), 0);
		ctlr->ntd = Ntd;
		ctlr->rd = ialloc(sizeof(D)*Nrd, 256);
		ctlr->rb = ialloc(Nrd*sizeof(Msgbuf*), 0);
		ctlr->nrd = Nrd;
		ctlr->dtcc = ialloc(sizeof(Dtcc), 64);
		rtl8169init(edev);
		ctlr->init = 1;
	}
	qunlock(&ctlr->alock);
