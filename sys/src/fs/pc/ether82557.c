...
command(Ctlr* ctlr, int c, int v)
{
	...
	if(timeo >= 100){
		ctlr->command = -1;
		iunlock(&ctlr->rlock);
		print("i82557: command %#ux %#ux timeout\n", c, v);
		return;
	}
...
rfdalloc(ulong link)
{
	...
	if(bp = mballoc(sizeof(Rfd), 0, Mbeth1)){
		rfd = (Rfd*)bp->data;
		rfd->field = 0;
		rfd->link = link;
		rfd->rbd = NullPointer;
...
attach(Ether* ether)
{
	...
	if(ctlr->state == 0){
		...
		if((ctlr->eeprom[0x03] & 0x0003) != 0x0003){
			snprint(ctlr->name, sizeof ctlr->name, "#l%dwatchdog", ether->ctlrno);
			userinit(watchdog, ether, ctlr->name);
		}
	}
	unlock(&ctlr->slock);
}
...
receive(Ether* ether)
{
	...
	ctlr = ether->ctlr;
	bp = ctlr->rfdhead;
	for(rfd = (Rfd*)bp->data; rfd->field & RfdC; rfd = (Rfd*)bp->data){
		/*
		...
		if(rfd->field & RfdOK){
			...
			if((count < ETHERMAXTU/4) && (pbp = mballoc(count, 0, Mbeth1))){
				memmove(pbp->data, bp->data+offsetof(Rfd, data[0]), count);
				pbp->count = count;
				rfd->count = 0;
				rfd->field = 0;
			}
			else if(xbp = rfdalloc(rfd->link)){
				bp->data += offsetof(Rfd, data[0]);
				bp->count = count;
		...
		((Rfd*)ctlr->rfdtail->data)->link = PADDR(bp->data);
