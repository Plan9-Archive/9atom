...
typedef struct Ctlr {
	...
	ushort	srom[0xB+1];
	uchar	sromea[Easize];	/* MAC address */
...
txstart(Ether* ether)
{
	...
	while(ctlr->ntq < ctlr->ntdr-1){
		...
		des->bp = bp;
		des->addr = PADDR(bp->data);
		ctlr->ntq++;
		coherence();
		des->cmdsts = Own | bp->count;
	...
	while((status = csr32r(ctlr, Risr)) != 0){
		...
		if(status & (Rxdesc|Rxok|Rxerr|Rxearly|Rxorn)){
			...
			while((cmdsts = des->cmdsts) & Own){
				...
				else if(bp = mballoc(Rbsz, 0, Mbeth1)){
					len = (cmdsts&Size)-4;
					if(len <= 0){
...
ctlrinit(Ether* ether)
{
	 ...
	 */
	des = ialloc((ctlr->nrdr+ctlr->ntdr)*sizeof(Des), 32);
	if(des == nil) {
	...
	for(des = ctlr->rdr; des < &ctlr->rdr[ctlr->nrdr]; des++){
		des->bp = mballoc(Rbsz, 0, Mbeth1);
		des->cmdsts = Rbsz;
		des->addr = PADDR(des->bp->data);
		if(last != nil)
			last->next = PADDR(des);
		last = des;
	}
