...
typedef struct {
	...
	Msgbuf*	bp;			/* Msgbuf containing bufptr */
...
static Lock dp83820rblock;		/* free receive Msgbufs */
static Msgbuf* dp83820rbpool;
...
/*
 * return free Msgbuf+buffer ready for input of up to MTU bytes.
...
dp83820rballoc(Desc* desc)
{
	...
	if(desc->bp == nil){
		...
		if((bp = dp83820rbpool) == nil){
			...
			desc->bp = nil;
			desc->cmdsts = Own;
			print("dp83820rballoc: out of buffers\n");
			return nil;
		}
		...
		desc->bufptr = PCIWADDR(bp->data);
		desc->bp = bp;
	}
	else{
		bp = desc->bp;
		bp->count = 0;
		bp->data = bp->xdata+256;
	}
...
dp83820cfg(Ctlr* ctlr)
{
	...
	if(!(cfg & Dupsts)){
		ctlr->rxcfg |= Rxfd;
		ctlr->txcfg |= Csi|Hbi;
		print("83820: full duplex, ");
	}
	else{
		ctlr->rxcfg &= ~Rxfd;
		ctlr->txcfg &= ~(Csi|Hbi);
		print("83820: half duplex, ");
	}
...
dp83820attach(Ether* edev)
{
	...
	ctlr->nrb = Nrb;
	ctlr->ntd = Ntd;
	ctlr->alloc = ialloc((ctlr->nrd+ctlr->ntd)*sizeof(Desc) + 7, 0);

	/*
	 * allocate receive Msgbufs+buffers, add all to receive Msgbuf+buffer pool
	 */
	for(ctlr->nrb = 0; ctlr->nrb < Nrb; ctlr->nrb++){
		bp = mballoc(Rbsz, 0, Mbeth1);
		bp->flags |= Mbrcvbuf;
		bp->free = dp83820rbfree;	/* to be called via freeb() */
		dp83820rbfree(bp);
	}

	/* attaches a receive Msgbuf+buffer to each receive Desc */
	dp83820init(edev);
...
//			edev->oerrs++;
		...
		desc->bp->next = bp;	/* chain transmitted Msgbufs together */
		bp = desc->bp;
		desc->bp = nil;		/* unlink them from Descs */

		ctlr->ntq--;
	}
	/* free Msgbufs+buffers comprising the packets just sent */
	ctlr->tdh = x;
	for(; bp; bp = next){
		next = bp->next;
		bp->next = 0;
	...
	while(ctlr->ntq < (ctlr->ntd-1)){
		...
		ctlr->ntq++;
		coherence();
		desc->cmdsts = Own|Intr|bp->count;	/* fire! */

		x = NEXT(x, ctlr->ntd);
	}
	if (ctlr->ntq >= ctlr->ntd-1)
		print("83820: xmit q full, Ntd=%d\n", Ntd);
	if(x != ctlr->tdt){
		ctlr->tdt = x;
		r = csr32r(ctlr, Cr);
		...
		if(isr & (Rxorn|Rxidle|Rxearly|Rxerr|Rxdesc|Rxok)){
			...
			while((cmdsts = desc->cmdsts) & Own){
				if((cmdsts & Ok) && desc->bp != nil){
					/* unlink rcv. Msgbuf from Desc */
					bp = desc->bp;
					desc->bp = nil;
					/* store its length, add to input q */
					bp->count += desc->cmdsts & SizeMASK;
					etheriq(edev, bp);
					rcvd++;
				}
				/* replace rcv. Msgbuf just detached from Desc */
				if (dp83820rballoc(desc) == nil)
					print(
...
reread:
	...
	if(ctlr->eepromsz == 0){
		...
		ctlr->eeprom = ialloc((1<<ctlr->eepromsz)*sizeof(ushort), 0);
		goto reread;
	}
