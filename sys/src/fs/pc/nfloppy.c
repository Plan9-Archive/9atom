...
Devsize
floppyseek(Floppy *dp, Devsize off)
{
	floppyon(dp);
	floppypos(dp, off);
	if(dp->cyl == dp->tcyl){
...
floppydev(Device *d)
{
	...
	i = d->wren.ctrl;
	if(i >= Maxfloppy)
		panic("floppy: bad drive %Z\n", d);
	return fl.d + i;
}

Off
flread(Device *dv, Devsize b, void *a)
{
	int tries, dev;
	uchar *aa;
	Floppy *dp;
	Off rv, i, n, nn, off, offset, sec;

	dp = floppydev(dv);
	dev = dv->wren.ctrl;
	off = b*(RBUFSIZE/c2b[fl.stat[6]]);
	if(floppyseek(dp, off) == -1)
		return -1;
	dp->len = RBUFSIZE;
	qlock(&fl);
...
Off
flwrite(Device *dv, Devsize b, void *a)
{
	int tries, dev, n;
	uchar *aa;
	Floppy *dp;
