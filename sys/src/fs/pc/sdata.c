...
ataverify(Drive *dp)
{
	...
	for (n = 0; n < RBUFSIZE; n++)
		buf[n] = n;
	nb = atawrite(dev, buf, RBUFSIZE, RBUFSIZE/dp->secsize);
	if (nb != RBUFSIZE)
		print("short write of %d bytes to block 1\n", nb);
	else {
		for (n = 0; n < RBUFSIZE; n++)
			buf[n] = ~n;
		nb = ataread(dev, buf, RBUFSIZE, RBUFSIZE/dp->secsize);
		if (nb != RBUFSIZE)
...
//		free(drive);
	...
	if((sdev = ialloc(sizeof(SDev), 0)) == nil){
//		free(ctlr);
//		free(drive);
		goto release;
	}
...
ataid(SDev* sdev)
{
	...
	while(sdev){
		if(sdev->ifc == &sdataifc){
			...
			snprint(ctlr->name, sizeof ctlr->name, "sd%c", sdev->idno);
			sdev->name = ctlr->name;
		}
		sdev = sdev->next;
	}
...
#define ALIGN	(4 * 1024)
	...
	dprint("ata irq %d %ux %s\n", ctlr->irq, ctlr->tbdf, name);
...
/* zero indicates failure; only otherinit() cares; why the heck doesn't devsd do this?? */
SDpart*
findpart(Drive **d, int driveno, char *name)
{
	SDunit *u;
	int i;
	...
	if(strcmp(name, "disk") == 0)
		name = "data";
	dprint("findpart(%d, %s)\n", driveno, name);
	if((*d = atadriveprobe(driveno)) == nil)
		return 0;

	u = sdgetunit(sdevs[driveno/NCtlrdrv], driveno%NCtlrdrv);
	for(i = 0; i < u->npart; i++)
		if(strcmp(u->part[i].name, name) == 0)
			return u->part+i;
	return 0;
}

int
setatapart(int driveno, char *name)
{
	Drive *d;
	SDpart *p;

	p = findpart(&d, driveno, name);
	if(p){
		dprint("...set to [%lld..%lld]\n", p->start, p->end);
		d->currpart = p;
		return 1;
	}
	return 0;
...
vlong
getatapartoff(int driveno, char *name)
{
	Drive *d;
	SDpart *p;
...
ataxfer(Drive *dp, int inout, Devsize start, long bytes)
{
	...
	start += dp->currpart->start*dp->secsize;
	if (start%secsize != 0)
...
Off
ataread(int driveno, void *a, long n, Devsize off)
{
	int skip;
	Off rv, i;
	uchar *aa = a;
	...
	skip = off % dp->secsize;
	for(rv = 0; rv < n; rv += i){
		i = ataxfer(dp, Read, off+rv-skip, n-rv+skip);
		if(i == 0)
			break;
		if(i < 0)
			return -1;
...
Off
atawrite(int driveno, void *a, long n, Devsize off)
{
	Off rv, i, partial;
	uchar *aa = a;
	Drive *dp;
	...
	partial = off % dp->secsize;
	if(partial){
		if (ataxfer(dp, Read, off-partial, dp->secsize) < 0)
			return -1;
		...
		if(ataxfer(dp, Write, off-partial, dp->secsize) < 0)
			return -1;
	} else
		rv = 0;
	 ...
	 */
	if(partial){
		if(ataxfer(dp, Read, off+rv, dp->secsize) < 0)
			return -1;
		memmove(dp->buf, aa+rv, partial);
		if(ataxfer(dp, Write, off+rv, dp->secsize) < 0)
			return -1;
		rv += partial;
	}
	return rv;
...
ideinit(Device *d)
{
	...
	d->private = dp = atadriveprobe(driveno);
	if(dp == 0)
		return;
	...
	if(dp->lba)
		lba = "L";
	s = dp->sectors;
	print("\t\t" "%lld %d sectors/%lld blocks %sLBA\n", s, dp->secsize, idesize(d), lba);
}
...
ideread(Device *d, Devsize b, void *c)
{
	...
	idprint("h%d read(%p, %lld, %p)\n", drriveno, d, (Wideoff)b, c);
	return ataread(driveno, c, RBUFSIZE, b*RBUFSIZE) != RBUFSIZE;
}
...
idewrite(Device *d, Devsize b, void *c)
{
	...
	idprint("h%d write(%p, %lld, %p)\n", drriveno, d, (Wideoff)b, c);
	return atawrite(driveno, c, RBUFSIZE, b*RBUFSIZE) != RBUFSIZE;
}
