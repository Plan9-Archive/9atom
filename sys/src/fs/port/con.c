...
found:
	...
	if(p == 0)
		p = "";
	snprint(buf, sizeof buf, "dumpctl %s", p);
	cmd_exec(buf);
}
...
cmd_stata(int, char *[])
{
	...
	print("	rah  =%W\n", &cons.brahead);
	print("	init =%W\n", &cons.binit);
	print("	bufs =    %3ld sm %3ld lg %ld nreseq\n",
...
//		cons.nwormhit, cons.nwormmiss);
	print("	ticks=	%uld	%uld	%uld\n", cons.cwio[0], cons.cwio[1], cons.cwio[2]);
}
...
cmd_cfs(int argc, char *argv[])
{
	...
	if(err = con_attach(FID1, "adm", fs->name))
		panic("FID1 attach to root %s (forgot to ream?)", errstr9p[err]);
	cons.curfs = fs;
...
installcmds(void)
{
	...
	cmd_install("devcopy", "from to -- copy device", cmd_devcopy);
	...
	cmd_install("rawcopy", "from to -- copy device", cmd_rawcopy);
	...
	cmd_install("writeconf", "-- write configuration", cmd_writeconf);

	attachflag = flag_install("attach", "-- attach calls");
	chatflag = flag_install("chat", "-- verbose");
	allowflag = flag_install("allow", "-- allow mode");
