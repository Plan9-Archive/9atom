...
static struct {
	...
	"pc", "#f/fd1disk", -1, 512,
	"amd64",  "#S/%s%x/9fat", -1, 512,
	"amd64", "#S/%s%x/nvram",	0, 512,
...
static char*
readcons(char *prompt, char *def, int raw, char *buf, int nbuf, int nilok)
{
	...
	if((flag&(NVwrite|NVwritemem)) || (err && (flag&NVwriteonerr))){
		if (!(flag&NVwritemem)) {
			readcons("authid", nil, 0, safe->authid,
					sizeof safe->authid, 0);
			readcons("authdom", nil, 0, safe->authdom,
					sizeof safe->authdom, 0);
			readcons("secstore key", nil, 1, safe->config,
					sizeof safe->config, 1);
			for(;;){
				if(readcons("password", nil, 1, in, sizeof in, 0)
				    == nil)
					goto Out;
 ...
 *  and I'd prefer not to waste the space on something that
