...
static	int	armuboot(int, Fhdr*, ExecHdr*);
...
ExecTable exectab[] =
{
	...
	{ (143<<16)|0413,		/* (Free|Net)BSD Arm */
		...
		leswal,
		armdotout },
	{ 0x27051956,			/* Arm Das U-Boot image */
		"arm u-boot image",
		nil,
		FARM,
		0,
...
/*
 * ARM Das U-Boot header.
 */
static int
armuboot(int fd, Fhdr *fp, ExecHdr *)
{
	long hdr[16];

	if (pread(fd, hdr, sizeof hdr, 0) < 0)
		return 0;
	fp->type = FARMB;
	fp->txtaddr = beswal(hdr[5]);
	fp->name = "arm u-boot image";
	fp->dataddr = beswal(hdr[4]);
	fp->hdrsz = 0;			/* header stripped */
	return 1;
}
