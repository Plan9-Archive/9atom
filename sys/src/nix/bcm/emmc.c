	...
	if(write){
		if(PIOwrite){
			while(len > 0){
				...
				now = m->ticks;
				while(((i = r[Interrupt])&(Writerdy|Err|Datadone)) == 0)
					if(m->ticks-now > 8*HZ){
						print("emmcio: (%d) no Writerdy intr %ux stat %ux\n", len, r[Interrupt], r[Status]);
						error(Eio);
					}
				if(i&(Err|Datadone)){
					print("emmcio: (%d) write error intr %ux stat %ux\n", len, r[Interrupt], r[Status]);
					WR(Interrupt, i);
					error(Eio);
				}
				if(i)
		...
		if(PIOread){
			while(len > 0){
				...
				while(((i = r[Interrupt])&(Readrdy|Err|Datadone)) == 0)
					if(m->ticks-now > 3*HZ){
						print("emmcio: (%d) no Readrdy intr %ux stat %ux\n", len, r[Interrupt], r[Status]);
						error(Eio);
					}
				if(i&(Err|Datadone)){
					print("emmcio: (%d) read error intr %ux stat %ux\n", len, r[Interrupt], r[Status]);
					WR(Interrupt, i);
					error(Eio);
				}
				if(i)
