...
chanwait(Ep *ep, Ctlr *ctlr, Hostchan *hc, int mask)
{
	...
	restart:
		...
		do{
			if(((intr = hc->hcint)&Chhltd) != 0){
				if((ointr != Ack && ointr != (Ack|Xfercomp)) ||
				   intr != (Ack|Chhltd|Xfercomp) || (now - start) > 60)
		...
		dprint("ep%d.%d halting channel %8.8ux hcchar %8.8ux grxstsr %8.8ux gnptxsts %8.8ux hptxsts %8.8ux\n", ep->dev->nb, ep->nb, intr, hc->hcchar, r->grxstsr, r->gnptxsts, r->hptxsts);
		mask = Chhltd;
		hc->hcchar |= Chdis;
		start = m->ticks;
		...
		if(dir == Epin && ep->ttype == Tbulk && n == nleft){
			...
			if(nt != n){
				if(n == ((nt+3)&~3))
					...
					print("usbotg: intr %8.8ux dma %8.8ux-%8.8ux hctsiz %8.8ux-%8.ux\n",
...
eptrans(Ep *ep, int rw, void *a, long n)
{
	...
	if(rw == Read && ep->ttype == Tbulk){
		...
		do{
			...
			if(m > ep->maxpkt)
				m = ep->maxpkt;
			m = chanio(ep, hc, Epin, ep->toggle[rw], (char*)a + sofar, m);
			ep->toggle[rw] = hc->hctsiz & Pid;
			sofar += m;
		}while(sofar < n && m == ep->maxpkt);
		n = sofar;
	}else{
		n = chanio(ep, hc, rw == Read? Epin : Epout, ep->toggle[rw], a, n);
		ep->toggle[rw] = hc->hctsiz & Pid;
	}
	chanrelease(ep, hc);
	poperror();
...
portreset(Hci *hp, int port, int on)
{
	...
	tsleep(&up->sleep, return0, 0, Enabledelay);
	s = r->hport0;
	if((b = (s&(Prtconndet|Prtenchng|Prtovrcurrchng))) != 0)
		r->hport0 = Prtpwr | b;
	dprint("usbotg reset=%d; sts %#x\n", on, s);
	if((s&Prtena) == 0)
