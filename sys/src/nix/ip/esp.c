...
print("esp: cipher failed %I -> %I!%ld: %s\n", vers.raddr, vers.laddr, vers.spi, up->errstr);
	...
	if(ckwindow(ecb, vers.seq) == -1){
		espstat(esp, InReplayErrors, 1);
		qunlock(c);
print("esp: window fail %I -> %I!%ld: %lux %lux %.64llub\n", vers.raddr, vers.laddr, vers.spi, vers.seq, ecb->w0, ecb->w);
		netlog(f, Logesp, "esp: window fail %I -> %I!%ld: %lux %lux\n",
			vers.raddr, vers.laddr, vers.spi, vers.seq, ecb->w0);
		freeb(bp);
		return;
	}
