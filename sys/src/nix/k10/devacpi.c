...
static Atable* acpimcfg(uchar*, int);
 ...
 */
static long
regio(Reg *r, void *p, usize len, uintptr off, int iswr)
{
	Regio rio;
	uintptr rp;
...
findmap(uintmem pa0, uint size)
{
	...
	o = pa0 - (pa0 & ~(PGSZ-1));
	pa = pa0 - o;
	size = ROUNDUP(size+o, PGSZ);

	if(pa+size < 1ull*MiB)
		return KADDR(pa);
	if(pa < 1ull*MiB)
		return nil;
	for(p = pa; p < pa+size; p += PGSZ){
		for(i = 0; i < smap.n; i++)
			if(p == smap.map[i])
				break;
		if(i == smap.n){
			if(smap.n == nelem(smap.map))
				panic("smap.map too small %d", smap.n);
			if(vmap(p, PGSZ) == nil)
				return nil;
			smap.map[smap.n++] = p;
		}
...
sdtmap(uintmem pa, int *n, int cksum)
{
	...
	sdt = findmap(pa, sizeof *sdt);
	if(sdt == nil){
		DBG("acpi: findmap: nil\n");
		return nil;
	}
	*n = l32get(sdt->length);
...
if(*n>200000){
		...
		memmove(tsig, sdt, 4);
		tsig[4] = 0;
		print("%s (too big %d)", tsig, *n);
		return nil;
}

	sdtunmap(sdt, sizeof(Sdthdr));
	if((sdt = findmap(pa, *n)) == nil){
		DBG("acpi: findmap: nil\n");
		return nil;
	}
...
print("a SDTMAP %#P → %#p	(%d)\n", pa, sdt, sizeof(Sdthdr));
	if(sdt == nil){
		DBG("acpi: vmap1: nil\n");
		return nil;
	}
...
acpimcfg(uchar *v, int)
{
	...
	for(i = offsetof(Mcfg, mcfgd[0]); i < n; i += sizeof(Mcfgd)){
		DBG("acpi: addr %#.16llux segno %d sbno %d ebno %d\n",
...
acpihpet(uchar *v, int)
{
	...
	h = (Hpet*)v;
	g = &h->addr;
	addr = g->addr; //l64get(g->addr);
	minticks = l16get(h->minticks);

	print("acpi: hpet id %#ux addr %d %d %d %d %#p seqno %d ticks %d attr %#ux\n",
		l32get(h->id), g->spc, g->len, g->off, g->accsz,
		addr, h->seqno, minticks, h->attr);

//	hpetinit(h->seqno, addr, minticks);
	return nil;
}
...
machcolor(uint machno)
{
	...
	if(machno >= MACHMAX)
		return -1;
	if((mp = sys->machptr[machno]) == nil)
		return -1;
	for(sl = srat; sl != nil; sl = sl->next)
		if(sl->type == SRlapic && sl->lapic.apic == mp->apicno)
			return sl->lapic.dom;
	return -1;
}
...
dumpmadt(Madt *apics)
{
	...
	DBG("acpi: madt lapic paddr %P pcat %d:\n", apics->lapicpa, apics->pcat);
...
acpirsdptr(void)
{
	...
	if((xsdt->p[0] != 'R' && xsdt->p[0] != 'X') || memcmp(xsdt->p+1, "SDT", 3) != 0){
		DBG("acpi: xsdt sig: %.4s\n", (char*)xsdt->p);
		sdtunmap(xsdt, xsdt->len);
		free(xsdt);
		xsdt = nil;
		return;
	}
