...
ethercheck(Ether *e)
{
	...
	c = e->ctlr;
	if (e->link && c->starttime != 0 &&
	    5 + Ticks - c->starttime > Etherstuck*HZ) {
		c->starttime = 0;
...
//		assert(b->ref == 1);
	...
	ctlr->rx = mallocalign(Nrx * sizeof(Rx), CACHELINESZ, 0, 0);
	if(ctlr->rx == nil)
	...
	ctlr->tx = mallocalign(Ntx * sizeof(Tx), CACHELINESZ, 0, 0);
	if(ctlr->tx == nil)
