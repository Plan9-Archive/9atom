...
void	mmuidmap(uintmem phys, int mbs);

Nandtab *
findflash(Flash *f, uintmem pa, uchar *id4p)
{
	int i;
	ulong sts;
...
ctlrwait(Flash *f)
{
	...
	for (;;) {
		...
		for(cnt = 0; cnt < 100; cnt++){
			if((nandread(f) & Srdymask) == Srdymask)
				break;
			microdelay(50);
		}
		nandcmd(f, Readstatus);
...
static void
fluncachepage(Flash *f, ulong pageno)
{
	Flashregion *r;
	...
	if(cache.pageno != pageno)
		return;
print("uncache page %#lux  %#lux \n", pageno, pageno<<r->pageshift);
	cache.pageno = ~0;
}
...
erasezone(Flash *f, Flashregion *r, ulong offset)
{
	...
	/* flush cache */
	for(i = 0; i < r->erasesize / r->pagesize; i++)
		fluncachepage(f, page + i);
...
write(Flash *f, ulong offset, void *buf, long n)
{
	...
	if(n < 0)
		panic("flashkw: write: negative count %ld", n);
	un = n;
	assert(r->pagesize != 0);
	pgsize = r->pagesize;
