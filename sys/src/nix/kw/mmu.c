	...
	for (va = i = 0; i < 4096; i++) {
		...
		if (ISHOLE(pte)) {
			if (endva != 0) {	/* open range? close it */
				iprint("l1 maps va (%#p-%#p) -> pa %#P type %#ux\n",
...
mmuinit(void)
{
	...
	for (i = 0; i < 1024/4; i++)
		l2[L2X(VIRTIO + i*PGSZ)] = (PHYSIO + i*PGSZ)|L2AP(Krw)|Small;
...
void
putmmu(uintptr va, uintmem pa, Page* page)
{
	int x;
	Page *pg;
	PTE *l1, *pte;
...
uintptr
mmukmap(uintptr va, uintmem pa, usize size)
{
	int x;
	PTE *pte;
...
uintptr
mmukunmap(uintptr va, uintmem pa, usize size)
{
	int x;
	PTE *pte;
